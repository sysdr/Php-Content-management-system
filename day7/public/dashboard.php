  ...
  <style>
    * { box-sizing: border-box; }
    body {
      ...
      margin: 0; padding: 0;
      background: linear-gradient(160deg, #0d2137 0%, #1a3a52 40%, #0f2d44 100%);
      min-height: 100vh; color: #e8eef2; line-height: 1.5;
    }
    .container { max-width: 920px; margin: 0 auto; padding: 1.5rem; }
    header {
      background: linear-gradient(90deg, #0e639c 0%, #1177bb 50%, #0d5689 100%);
      color: #fff; padding: 1.35rem 0; margin-bottom: 1.5rem;
    ...
    .metric-card:nth-child(1) {
      background: linear-gradient(145deg, rgba(34, 197, 94, 0.22), rgba(34, 197, 94, 0.06));
    ...
    .metric-card:nth-child(3) {
      background: linear-gradient(145deg, rgba(168, 85, 247, 0.22), rgba(168, 85, 247, 0.06));
    ...
    /* Different operation UI: command-style trigger + description */
    .ops-panel {
      display: flex;
      flex-wrap: wrap;
      align-items: center;
...
<body>
  <header>
    <div class="container">
      <h1>Resource Cleanup — Dashboard (Day 7)</h1>
      <p>Simulate request lifecycle and __destruct cleanup. Trigger a run below to refresh metrics.</p>
    </div>
  </header>
  <div class="container">
    <section>
      ...
      <div class="metrics">
        <div class="metric-card">
          <h3>Demo runs</h3>
          <div class="metric-value" id="metric-runs"><?= $requestRuns ?></div>
          <div class="metric-note">Cycles executed</div>
        </div>
        <div class="metric-card">
          <h3>Destruct calls (last run)</h3>
          <div class="metric-value" id="metric-destruct"><?= $destructCalls ?></div>
          <div class="metric-note">Resources released via __destruct</div>
        </div>
        <div class="metric-card">
          <h3>Total requests simulated</h3>
          <div class="metric-value" id="metric-total"><?= $totalRequests ?></div>
          <div class="metric-note">3 requests per cycle</div>
        </div>
      </div>
    </section>
    <section>
      ...
      <div class="ops-panel">
        <div class="ops-desc">
          Run one resource-cleanup cycle (3 simulated requests). Each request acquires a resource and releases it via <code>__destruct</code> when the handler ends.
        </div>
        <a href="#" role="button" id="op-trigger" class="ops-trigger" aria-label="Execute resource cleanup simulation">
...
(function() {
  ...
  function refreshMetrics() {
    fetchStats().then(function(s) {
      document.getElementById('metric-runs').textContent = s.request_runs != null ? s.request_runs : 0;
      document.getElementById('metric-destruct').textContent = s.last_destruct_calls != null ? s.last_destruct_calls : 0;
      document.getElementById('metric-total').textContent = s.total_requests_simulated != null ? s.total_requests_simulated : 0;
    });
  }
  triggerEl.addEventListener('click', function(e) {
    ...
    fetch(origin + '/run_demo.php')
      .then(function(r) { return r.json(); })
      .then(function(d) {
        if (d.ok) {
          setStatus('Cycle complete. Metrics updated.', 'ok');
          refreshMetrics();
        } else {
          setStatus(d.error || 'Error', 'err');
        }
      })
      .catch(function() {
